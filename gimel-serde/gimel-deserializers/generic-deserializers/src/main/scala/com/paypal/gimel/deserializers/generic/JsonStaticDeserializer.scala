...
import com.paypal.gimel.deserializers.common.Deserializer
import com.paypal.gimel.deserializers.common.utils.SQLDataTypesUtils
    ...
    if (!dataframe.columns.contains(conf.columnToDeserialize)) {
      throw new IllegalArgumentException(
        s"""
           ...
           | Default value of which is "${GenericDeserializerConstants.columnToDeserialize}".
         """.stripMargin
      )
    } else {
