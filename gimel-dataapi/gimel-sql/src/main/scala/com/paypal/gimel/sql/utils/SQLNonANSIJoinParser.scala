...
package com.paypal.gimel.sql.utils

import scala.collection.immutable.Map

import com.paypal.gimel.logger.Logger
import com.paypal.gimel.sql.GimelQueryUtils
...
object SQLNonANSIJoinParser {
  ...
  def checkIfNonANSIJoin(tokenAndStuff: Map[Int, (String, Boolean)], startPoint: Int): Boolean = {
    ...
    val hasNonAnsiJoin = (length >= indexOfFrom + 2) match {
      case false => false
      // Check if there is a ',' followed by from tbl.
      // Example => [ from tbl1 , tbl2 ] results TRUE
      // Example => [ from tbl1 t, tbl2 t2 ] results TRUE
      case true => (tokenAndStuff(indexOfFrom + 1)._1 == "," || tokenAndStuff(indexOfFrom + 2)._1 == ",")
    }
    hasNonAnsiJoin
  }

  /**
    * Given a SQL - determine if non-ANSI standard join (orcale like) is in the SQL
    *
    * @param sql SQL String
    * @return True / False
    */
  def isSQLNonANSIJoin(sql: String): Boolean = {
    val pickings: Map[Int, (String, Boolean)] = GimelQueryUtils.tokensWithPickings(GimelQueryUtils.tokenizeSql(sql).toSeq, Seq("from"))
  ...
  def getSourceTablesFromNonAnsi(sql: String): Seq[String] = {
    ...
    val tokensAsMap1: Map[Int, (String, Boolean)] = GimelQueryUtils.tokensWithPickings(GimelQueryUtils.tokenizeSql(sql).toSeq, Seq("from"))
