...
class GimelServiceUtilitiesTest extends FunSuite {
  ...
  test("getObjectPropertiesForSystem") {
    println("1. storageTypeName=HIVE, dataset=Hive.Horton.default.flights")
    assert(gimelServices.getObjectPropertiesForSystem("HIVE", "Hive.Horton.default.flights")
    ...
    assert(gimelServices.getObjectPropertiesForSystem("TERADATA", "Teradata.Simba.flights_db.flights")
