...
class HiveUtils {
  ...
  def write(dataSet: String, dataFrame: DataFrame, sparkSession: SparkSession, dataSetProps: Map[String, Any]): DataFrame = {
    ...
    val checkPCatalogDB = checkIfCatalogTable(dataSet);
    val location = if (checkPCatalogDB) {
      datasetProps.props.getOrElse(HiveConfigs.dataLocation, "")
    } else {
      datasetProps.props.getOrElse(CatalogProviderConstants.PROPS_LOCATION, "")
    }
    val hiveDataSetName = if (checkPCatalogDB) {
      datasetProps.props(HiveConfigs.hiveDBName) + "." + datasetProps.props(HiveConfigs.hiveTableName)
    } else {
      dataSet
    }
    val partitionColumns: Array[String] = datasetProps.partitionFields.map(_.fieldName)
    val partKeys = partitionColumns.mkString(",")
    val fieldNames = datasetProps.fields.map(_.fieldName).mkString(",")

    val numColsInParts = partitionColumns.length
    val updatedPartKeys = dataFrame.schema.map { x => x.name }.reverse.take(numColsInParts).mkString(",")
  ...
  /**
    *
    * @param dataset Name of the UDC or PCATALOG Data Set
  ...
  /**
    * Create hive table by prparing the CREATE sql statement
    *
  ...
  def getUniqueID(sparkUser: String, action: String, hiveDataSetName: String): String = {
    ...
    uniqueID.take(GimelConstants.HIVE_MAX_TABLE_NAME_SIZE - hiveTableLen)
  }
  ...
  def authenticateTableAndLocationPolicy(dataset: String, dataSetProps: Map[String, Any], sparkSession: SparkSession, operation: String): Unit = {
    def MethodName: String = new Exception().getStackTrace.apply(1).getMethodName
    logger.info(" @Begin --> " + MethodName)
